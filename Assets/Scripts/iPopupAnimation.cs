...
public delegate void MethodPopOpenClose(iPopupAnimation pop);
...
public class iPopupAnimation : MonoBehaviour
{
    ...
    public void paint(float dt)
    {
        ...
        {
			popUpAlpha(position, alpha, dt);
        }
		else if (style== iPopupStyle.move)
        {
			popUpMove(position, dt);
        }
		else if (style ==iPopupStyle.zoom)
        {
			popUpZoom(position, alpha, scale, dt);
        }
		else if (style == iPopupStyle.zoomRotate)
		{
			popUpZoomRotate(position, degree, scale, dt);
		}
	}

	void popUpAlpha(Vector2 position, float alpha, float dt)
    {
	...
	void popUpMove(Vector2 position, float dt)
    {
		if (state == iPopupState.open)
		{
			aniDt += dt;
	...
	void popUpZoom(Vector2 position, float alpha, float scale, float dt)
    {
		if (state == iPopupState.open)
		{
			aniDt += dt;
	...
	void popUpZoomRotate(Vector2 position, float degree, float scale, float dt)
    {
		if (state == iPopupState.open)
		{
			aniDt += dt;
